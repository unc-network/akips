...
- Added a security audit workflow that runs `pip-audit` against the locked
  dependencies on changes and weekly, and enabled ruff's `S` rules, which
  port bandit's static security checks into the existing lint pass.
